...
def getTrainingText(cand, fname):
    ...
    for e in encodings:
        try:
            fh = codecs.open(os.path.join(corpusdir, cand, fname), 'r', encoding=e)
            fh.readlines()
            fh.seek(0)
        except UnicodeDecodeError:
            continue
        else:
            dfile = codecs.open(os.path.join(corpusdir, cand, fname), "r", encoding=e)
            s = dfile.read()
            dfile.close()
            return s
    raise Exception("No proper encoding was found for the file: " + fname)
...
def getUnknownText(fname):
    ...
    for e in encodings:
        try:
            fh = codecs.open(os.path.join(upath, fname), "r", encoding=e)
            fh.readlines()
            fh.seek(0)
        except UnicodeDecodeError:
            continue
        else:
            dfile = codecs.open(os.path.join(upath, fname), "r", encoding=e)
            s = dfile.read()
            dfile.close()
            return s
    raise Exception("No proper encoding was found for the file: " + fname)
